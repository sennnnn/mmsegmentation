...
class MLAModule(nn.Module):
    ...
    def forward(self, inputs):
        ...
        max_h = max([x.shape[2] for x in feat_list])
        max_w = max([x.shape[3] for x in feat_list])
        feat_list = [
            F.interpolate(
                feat,
                size=(max_h, max_w),
                mode='bilinear',
                align_corners=False) for feat in feat_list
        ]
