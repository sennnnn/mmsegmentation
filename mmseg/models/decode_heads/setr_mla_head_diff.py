...
class SETRMLAHeadDiff(BaseDecodeHead):
    ...
    def __init__(self, mla_channels=128, up_scale=4, **kwargs):
        super(SETRMLAHeadDiff, self).__init__(
    ...
    def forward(self, inputs):
        ...
        max_h = max([x.shape[2] for x in outs])
        max_w = max([x.shape[3] for x in outs])
        outs = [F.interpolate(x, size=(max_h, max_w), mode='bilinear', align_corners=self.align_corners)]
